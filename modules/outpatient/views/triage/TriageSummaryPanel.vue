...
watch(() => props.patient, async (p) => {
  ...
  if (p?.patient_id) {
    ...
    try {
      const res = await api.patientSummary(p.patient_id);
  ...
  <div class="block block-rounded block-bordered d-flex flex-column mb-0" style="overflow: hidden">
    <div class="block-header block-header-default" style="background: linear-gradient(135deg, #2c3e66, #1a2c4e)">
      <h3 class="block-title text-white fw-semibold">
        <i class="fa fa-chart-line me-1"></i> Patient Clinical Summary
      </h3>
      <div class="text-white-50 fs-xs fw-medium">Historical data to inform triage decision</div>
    </div>
    <div class="block-content flex-grow-1 p-3" style="max-height: 650px; overflow-y: auto; background-color: #ffffff;">
      ...
      <div v-else-if="!summary" class="d-flex flex-column justify-content-center align-items-center h-100 text-muted" style="min-height: 400px;">
        <i class="fa fa-notes-medical fa-3x mb-3 opacity-25"></i>
        <p class="fs-sm fw-medium">No historical data available for this patient</p>
      </div>
      ...
      <div v-else>
        ...
        <div class="mb-4">
          ...
          <div class="p-3 rounded" style="background: #f8fafc; border: 1px solid #eef3f8; transition: all 0.2s;">
            <div class="fw-semibold" style="color: #1f3b4c; font-size: 0.9rem;">{{ summary.overview?.full_name || "Patient" }}</div>
        ...
        <div class="mb-4">
          <div class="fw-bold fs-sm mb-2 d-flex align-items-center" style="color: #1e8f5e; border-left: 3px solid #1e8f5e; padding-left: 12px;">
            <i class="fa fa-info-circle me-2"></i> Clinical Notes
          </div>
          <div class="p-3 rounded" style="background: #f8fafc; border: 1px solid #eef3f8; transition: all 0.2s;">
            <div style="font-size: 0.75rem; color: #64748b;">
              💡 {{ summary.clinical_note || "Review patient history before triage. Pay attention to alerts and active conditions." }}
            </div>
          </div>
        </div>
      </div>
