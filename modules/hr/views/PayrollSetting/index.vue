...
const {
  ...
  initialSortBy: 'payroll_setting_id',
  initialSortDir: 'desc',
});

const tableColumns = [
  { attribute: "pay_cycle", label: "Pay Cycle" },
  { attribute: "currency", label: "Currency" },
  { attribute: "overtime_rate", label: "Overtime Rate" },
  { attribute: "tax_calculation_method", label: "Tax Calculation Method" },
...
const endpoints = {
  "list": "/hr/payroll-settings",
  "create": "/hr/payroll-setting",
  "view": "/hr/payroll-setting/{id}",
  "update": "/hr/payroll-setting/{id}",
  "delete": "/hr/payroll-setting/{id}"
};

function rowId(row) {
  return row.id ?? row.payroll_setting_id ?? row[Object.keys(row).find((key) => key.endsWith('_id'))];
}
...
function handleCreate() {
  ...
  if (!modalStore.useModal) return router.push({ name: 'hr/payroll-setting/create' });
...
async function handleView(row) {
  ...
  if (!modalStore.useModal) return router.push({ name: 'hr/payroll-setting/view', params: { id } });
  if (!id) return alertStore.show({ theme: 'danger', type: 'toast', message: 'Record id not found.' });

  openFormModal('View PayrollSetting', {}, true);
...
async function handleEdit(row) {
  ...
  if (!modalStore.useModal) return router.push({ name: 'hr/payroll-setting/update', params: { id } });
  if (!id) return alertStore.show({ theme: 'danger', type: 'toast', message: 'Record id not found.' });

  openFormModal('Edit PayrollSetting', {}, false);
  modalStore.setLoading(true);
...
async function handleDelete(row) {
  ...
  handleResponseAlert(alertStore, responseData.value, `PayrollSetting ${isRestore ? 'restored' : 'deleted'} successfully.`);
  await fetchRows();
}
...
async function handleSubmit(payload) {
  ...
  handleResponseAlert(alertStore, responseData.value, isEdit ? 'PayrollSetting updated successfully.' : 'PayrollSetting created successfully.');
...
<template>
  <div class="content">
    ...
    >
      <BaseBlock title="Payroll Settings" content-full>
        <template #options>
          <div class="d-flex align-items-center gap-2">
            <div class="input-group input-group-sm w-auto me-3">
              ...
              <input type="text" class="form-control border-start-0" placeholder="Search settings..." style="width: 200px" :value="searchQuery" @input="(e) => handleSearch(e.target.value)" />
            </div>
            ...
            <button type="button" class="btn btn-sm btn-primary" @click="handleCreate"><i class="fa fa-plus me-1"></i> New Setting</button>
          </div>
        </template>
          ...
          <template #card="{ row }">
            <div class="block block-rounded block-bordered h-100 mb-0 shadow-sm">
              <div class="block-content text-center py-4">
                ...
                <h4 class="mb-1">{{ row.pay_cycle || 'N/A' }} Cycle</h4>
                <p class="text-muted fs-sm mb-2">Currency: {{ row.currency }} | Overtime: {{ row.overtime_rate }}</p>
                <span 
                  class="badge" 
                  :class="row.status?.theme ? `bg-${row.status.theme}` : (row.status?.label === 'Active' ? 'bg-success' : 'bg-danger')"
                >
