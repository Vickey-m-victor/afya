...
const tableColumns = [
  ...
  { attribute: "gender_specific", label: "Gender Specific" },
  { attribute: "paid", label: "Paid" },
  { attribute: "requires_approval", label: "Requires Approval" },
...
function handleCreate() {
  ...
  if (!modalStore.useModal) return router.push({ name: 'hr/leave-type/create' });
...
async function handleDelete(row) {
  ...
  handleResponseAlert(alertStore, responseData.value, `LeaveType ${isRestore ? 'restored' : 'deleted'} successfully.`);
  await fetchRows();
}
...
async function handleSubmit(payload) {
  ...
  handleResponseAlert(alertStore, responseData.value, isEdit ? 'LeaveType updated successfully.' : 'LeaveType created successfully.');
...
<template>
  <div class="content">
    ...
    >
      <BaseBlock title="Leave Types" content-full>
        <template #options>
          <div class="d-flex align-items-center gap-2">
            <div class="input-group input-group-sm w-auto me-3">
              ...
              <input type="text" class="form-control border-start-0" placeholder="Search leave types..." style="width: 200px" :value="searchQuery" @input="(e) => handleSearch(e.target.value)" />
            </div>
            ...
            <button type="button" class="btn btn-sm btn-primary" @click="handleCreate"><i class="fa fa-plus me-1"></i> New LeaveType</button>
          </div>
        </template>
          ...
          <template #card="{ row }">
            <div class="block block-rounded block-bordered h-100 mb-0 shadow-sm">
              <div class="block-content text-center py-4">
                ...
                <p class="text-muted fs-sm mb-2">{{ row.default_days }} Days Default</p>
                <div class="d-flex gap-2 justify-content-center mt-2">
                  <span v-if="row.paid === 1 || row.paid === true || row.paid === 'Yes'" class="badge bg-success">Paid</span>
                  <span v-else class="badge bg-secondary">Unpaid</span>
                  <span v-if="row.requires_approval === 1 || row.requires_approval === true" class="badge bg-warning">Requires Approval</span>
                </div>
              </div>
