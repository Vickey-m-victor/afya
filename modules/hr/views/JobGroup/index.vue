...
function handleCreate() {
  ...
  if (!modalStore.useModal) return router.push({ name: "hr/job-group/create" });
...
async function handleDelete(row) {
  ...
  handleResponseAlert(
    alertStore,
    responseData.value,
    `JobGroup ${isRestore ? "restored" : "deleted"} successfully.`
  );
  await fetchRows();
}
...
async function handleSubmit(payload) {
  ...
  handleResponseAlert(
    alertStore,
    responseData.value,
    isEdit ? "JobGroup updated successfully." : "JobGroup created successfully."
  );
...
<template>
  <div class="content">
    ...
    >
      <BaseBlock title="Job Groups" content-full>
        <template #options>
          <div class="d-flex align-items-center gap-2">
            ...
            <button
              ...
              @click="handleCreate"
            >
              <i class="fa fa-plus me-1"></i> New JobGroup
            </button>
          </div>
        </template>
          ...
          <template #card="{ row }">
            ...
            >
              <div class="block-content text-center py-4">
                ...
                <h4 class="mb-1">{{ row.group_name }}</h4>
