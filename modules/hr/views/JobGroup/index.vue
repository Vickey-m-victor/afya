...
const endpoints = {
  "list": "/hr/job-groups",
  "create": "/hr/job-group",
  "view": "/hr/job-group/{id}",
  "update": "/hr/job-group/{id}",
  "delete": "/hr/job-group/{id}"
};

function rowId(row) {
  return row.id ?? row.job_group_id ?? row[Object.keys(row).find((key) => key.endsWith('_id'))];
}
...
function handleCreate() {
  ...
  if (!modalStore.useModal) {
    router.push({ name: 'hr/job-group/create' });
    return;
  }

  openFormModal('Create JobGroup', {}, false);
}
...
function handleView(row) {
  ...
  if (!modalStore.useModal) {
    const id = rowId(row);
    router.push({ name: 'hr/job-group/view', params: { id } });
    return;
  }

  openFormModal('View JobGroup', { ...row }, true);
}
...
function handleEdit(row) {
  ...
  if (!modalStore.useModal) {
    const id = rowId(row);
    router.push({ name: 'hr/job-group/update', params: { id } });
    return;
  }

  openFormModal('Edit JobGroup', { ...row }, false);
}
...
async function handleDelete(row) {
  ...
  handleResponseAlert(alertStore, responseData.value, 'JobGroup deleted successfully.');
  await fetchRows();
}
...
async function handleSubmit(payload) {
  ...
  handleResponseAlert(
    ...
    responseData.value,
    isEdit
      ? 'JobGroup updated successfully.'
      : 'JobGroup created successfully.'
  );
...
<template>
  <div class="content">
    <DataTable
      ...
      search-placeholder="Search job-group..."
      create-label="New JobGroup"
      :paginated="true"
