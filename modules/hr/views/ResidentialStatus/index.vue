...
const {
  ...
  initialSortBy: 'residential_status_id',
  initialSortDir: 'desc',
});
...
const endpoints = {
  "list": "/hr/residential-statuses",
  "create": "/hr/residential-status",
  "view": "/hr/residential-status/{id}",
  "update": "/hr/residential-status/{id}",
  "delete": "/hr/residential-status/{id}"
};

function rowId(row) {
  return row.id ?? row.residential_status_id ?? row[Object.keys(row).find((key) => key.endsWith('_id'))];
}
...
function handleCreate() {
  ...
  openFormModal('Create ResidentialStatus', {}, false);
}
...
function handleView(row) {
  ...
  openFormModal('View ResidentialStatus', { ...row }, true);
}
...
function handleEdit(row) {
  ...
  openFormModal('Edit ResidentialStatus', { ...row }, false);
}
...
async function handleDelete(row) {
  ...
  handleResponseAlert(alertStore, responseData.value, 'ResidentialStatus deleted successfully.');
  await fetchRows();
}
...
async function handleSubmit(payload) {
  ...
  handleResponseAlert(
    ...
    responseData.value,
    isEdit
      ? 'ResidentialStatus updated successfully.'
      : 'ResidentialStatus created successfully.'
  );
...
<template>
  <div class="content">
    <DataTable
      ...
      search-placeholder="Search residential-status..."
      create-label="New ResidentialStatus"
      :paginated="true"
