...
function handleCreate() {
  ...
  if (!modalStore.useModal) {
    router.push({ name: 'hr/statutory-rate/create' });
    return;
  }
...
async function handleView(row) {
  ...
  if (!modalStore.useModal) {
    const id = rowId(row);
    router.push({ name: 'hr/statutory-rate/view', params: { id } });
    return;
  }
...
async function handleEdit(row) {
  ...
  if (!modalStore.useModal) {
    const id = rowId(row);
    router.push({ name: 'hr/statutory-rate/update', params: { id } });
    return;
  }
...
async function handleDelete(row) {
  ...
  handleResponseAlert(alertStore, responseData.value, isRestore ? 'StatutoryRate restored successfully.' : 'StatutoryRate deleted successfully.');
  await fetchRows();
}
...
async function handleSubmit(payload) {
  ...
  handleResponseAlert(
    ...
    responseData.value,
    isEdit
      ? 'StatutoryRate updated successfully.'
      : 'StatutoryRate created successfully.'
  );
...
<template>
  <div class="content">
    <DataTable
      ...
      search-placeholder="Search statutory-rate..."
      create-label="New StatutoryRate"
      :paginated="true"
