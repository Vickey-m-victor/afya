...
const {
  ...
  initialSortBy: 'statutory_rate_id',
  initialSortDir: 'desc',
});

const tableColumns = [
  { attribute: "name", label: "Name" },
  { attribute: "authority", label: "Authority" },
  { attribute: "type", label: "Type" },
  { attribute: "employee_rate", label: "Employee Rate" },
  { attribute: "employer_rate", label: "Employer Rate" },
  { attribute: "effective_from", label: "Effective From" },
  { attribute: "effective_to", label: "Effective To" },
...
const endpoints = {
  "list": "/hr/statutory-rates",
  "create": "/hr/statutory-rate",
  "view": "/hr/statutory-rate/{id}",
  "update": "/hr/statutory-rate/{id}",
  "delete": "/hr/statutory-rate/{id}"
};

function rowId(row) {
  return row.id ?? row.statutory_rate_id ?? row[Object.keys(row).find((key) => key.endsWith('_id'))];
}
...
function handleCreate() {
  ...
  if (!modalStore.useModal) return router.push({ name: 'hr/statutory-rate/create' });
  openFormModal('Create StatutoryRate', {}, false);
}

async function handleView(row) {
  modalMode.value = 'view'; modalStore.toggleModalUsage(true);
  const id = rowId(row);
  if (!modalStore.useModal) return router.push({ name: 'hr/statutory-rate/view', params: { id } });
  ...
  const payload = responseData.value?.dataPayload || responseData.value || {};
  openFormModal('View StatutoryRate', payload.data || {}, true);
}

async function handleEdit(row) {
  modalMode.value = 'edit'; modalStore.toggleModalUsage(true);
  const id = rowId(row);
  if (!modalStore.useModal) return router.push({ name: 'hr/statutory-rate/update', params: { id } });
  if (!id) return alertStore.show({ theme: 'danger', type: 'toast', message: 'Record id not found.' });
  ...
  const payload = responseData.value?.dataPayload || responseData.value || {};
  openFormModal('Edit StatutoryRate', payload.data || {}, false);
}
...
async function handleDelete(row) {
  ...
  handleResponseAlert(alertStore, responseData.value, `StatutoryRate ${isRestore ? 'restored' : 'deleted'} successfully.`);
  await fetchRows();
}
...
async function handleSubmit(payload) {
  ...
  handleResponseAlert(alertStore, responseData.value, isEdit ? 'StatutoryRate updated successfully.' : 'StatutoryRate created successfully.');
...
<template>
  <div class="content">
    ...
    >
      <BaseBlock title="Statutory Rates" content-full>
        <template #options>
          <div class="d-flex align-items-center gap-2">
            <div class="input-group input-group-sm w-auto me-3">
              ...
              <input type="text" class="form-control border-start-0" placeholder="Search rates..." style="width: 200px" :value="searchQuery" @input="(e) => handleSearch(e.target.value)" />
            </div>
            ...
            <button type="button" class="btn btn-sm btn-primary" @click="handleCreate"><i class="fa fa-plus me-1"></i> New Rate</button>
          </div>
        </template>
          ...
          <template #card="{ row }">
            <div class="block block-rounded block-bordered h-100 mb-0 shadow-sm">
              <div class="block-content text-center py-4">
                ...
                <p class="text-muted fs-sm mb-2">{{ row.authority }} | Type: {{ row.type }}</p>
                <div class="d-flex gap-2 justify-content-center mt-2">
                  <span class="badge bg-secondary">Emp: {{ row.employee_rate }}</span>
                  <span class="badge bg-secondary">Comp: {{ row.employer_rate }}</span>
                </div>
              </div>
