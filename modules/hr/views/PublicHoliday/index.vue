...
const tableColumns = [
  ...
  {
    "field": "double_pay_if_worked",
    "header": "Double Pay If Worked"
  },
  {
    "field": "description",
    "header": "Description"
...
function handleCreate() {
  ...
  if (!modalStore.useModal) {
    router.push({ name: 'hr/public-holiday/create' });
    return;
  }
...
function handleView(row) {
  ...
  if (!modalStore.useModal) {
    const id = rowId(row);
    router.push({ name: 'hr/public-holiday/view', params: { id } });
    return;
  }

  openFormModal('View PublicHoliday', { ...row }, true);
}
...
function handleEdit(row) {
  ...
  if (!modalStore.useModal) {
    const id = rowId(row);
    router.push({ name: 'hr/public-holiday/update', params: { id } });
    return;
  }

  openFormModal('Edit PublicHoliday', { ...row }, false);
}
...
async function handleDelete(row) {
  ...
  handleResponseAlert(alertStore, responseData.value, 'PublicHoliday deleted successfully.');
  await fetchRows();
}
...
async function handleSubmit(payload) {
  ...
  handleResponseAlert(
    ...
    responseData.value,
    isEdit
      ? 'PublicHoliday updated successfully.'
      : 'PublicHoliday created successfully.'
  );
...
<template>
  <div class="content">
    <DataTable
      ...
      search-placeholder="Search public-holiday..."
      create-label="New PublicHoliday"
      :paginated="true"
