...
const {
  ...
  initialSortBy: 'relationship_type_id',
  initialSortDir: 'desc',
});
...
const endpoints = {
  "list": "/hr/dependent-relationship-types",
  "create": "/hr/dependent-relationship-type",
  "view": "/hr/dependent-relationship-type/{id}",
  "update": "/hr/dependent-relationship-type/{id}",
  "delete": "/hr/dependent-relationship-type/{id}"
};

function rowId(row) {
  return row.id ?? row.dependent_relationship_type_id ?? row[Object.keys(row).find((key) => key.endsWith('_id'))];
}
...
function handleCreate() {
  ...
  if (!modalStore.useModal) {
    router.push({ name: 'hr/dependent-relationship-type/create' });
    return;
  }

  openFormModal('Create DependentRelationshipType', {}, false);
}
...
async function handleView(row) {
  ...
  if (!modalStore.useModal) return router.push({ name: 'hr/dependent-relationship-type/view', params: { id } });
  if (!id) return alertStore.show({ theme: 'danger', type: 'toast', message: 'Record id not found.' });

  openFormModal('View DependentRelationshipType', {}, true);
...
async function handleEdit(row) {
  ...
  if (!modalStore.useModal) return router.push({ name: 'hr/dependent-relationship-type/update', params: { id } });
  if (!id) return alertStore.show({ theme: 'danger', type: 'toast', message: 'Record id not found.' });

  openFormModal('Edit DependentRelationshipType', {}, false);
  modalStore.setLoading(true);
...
async function handleDelete(row) {
  ...
  handleResponseAlert(alertStore, responseData.value, isRestore ? 'DependentRelationshipType restored successfully.' : 'DependentRelationshipType deleted successfully.');
  await fetchRows();
}
...
async function handleSubmit(payload) {
  ...
  handleResponseAlert(
    ...
    responseData.value,
    isEdit
      ? 'DependentRelationshipType updated successfully.'
      : 'DependentRelationshipType created successfully.'
  );
...
<template>
  <div class="content">
    <DataTable
      title="DependentRelationshipType"
      :data="rows"
      ...
      search-placeholder="Search dependent-relationship-type..."
      create-label="New DependentRelationshipType"
      :paginated="true"
