...
function handleCreate() {
  ...
  if (!modalStore.useModal) {
    router.push({ name: 'hr/worker-union/create' });
    return;
  }
...
function handleView(row) {
  ...
  if (!modalStore.useModal) {
    const id = rowId(row);
    router.push({ name: 'hr/worker-union/view', params: { id } });
    return;
  }

  openFormModal('View WorkerUnion', { ...row }, true);
}
...
function handleEdit(row) {
  ...
  if (!modalStore.useModal) {
    const id = rowId(row);
    router.push({ name: 'hr/worker-union/update', params: { id } });
    return;
  }

  openFormModal('Edit WorkerUnion', { ...row }, false);
}
...
async function handleDelete(row) {
  ...
  handleResponseAlert(alertStore, responseData.value, 'WorkerUnion deleted successfully.');
  await fetchRows();
}
...
async function handleSubmit(payload) {
  ...
  handleResponseAlert(
    ...
    responseData.value,
    isEdit
      ? 'WorkerUnion updated successfully.'
      : 'WorkerUnion created successfully.'
  );
...
<template>
  <div class="content">
    <DataTable
      ...
      search-placeholder="Search worker-union..."
      create-label="New WorkerUnion"
      :paginated="true"
