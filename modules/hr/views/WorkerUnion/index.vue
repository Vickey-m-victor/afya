...
const {
  ...
  initialSortBy: 'union_id',
  initialSortDir: 'desc',
});

const tableColumns = [
  { attribute: "union_name", label: "Union Name" },
  { attribute: "union_code", label: "Union Code" },
  { attribute: "registration_number", label: "Registration Number" },
  { attribute: "contact_person", label: "Contact Person" },
  { attribute: "phone", label: "Phone" },
  { attribute: "email", label: "Email" },
  { attribute: "address", label: "Address" },
  { attribute: "monthly_dues", label: "Monthly Dues" },
  { attribute: "status", label: "Status", contentClass: "text-center" },
  { attribute: "created_by", label: "Created By" },
  { attribute: "updated_by", label: "Updated By" },
...
function rowId(row) {
  return row.id ?? row.worker_union_id ?? row[Object.keys(row).find((key) => key.endsWith('_id'))];
}
...
function handleCreate() {
  ...
  if (!modalStore.useModal) return router.push({ name: 'hr/worker-union/create' });
  openFormModal('Create WorkerUnion', {}, false);
}
...
async function handleDelete(row) {
  ...
  handleResponseAlert(alertStore, responseData.value, `WorkerUnion ${isRestore ? 'restored' : 'deleted'} successfully.`);
  await fetchRows();
}
...
async function handleSubmit(payload) {
  ...
  handleResponseAlert(alertStore, responseData.value, isEdit ? 'WorkerUnion updated successfully.' : 'WorkerUnion created successfully.');
...
<template>
  <div class="content">
    ...
    >
      <BaseBlock title="Worker Unions" content-full>
        <template #options>
          <div class="d-flex align-items-center gap-2">
            <div class="input-group input-group-sm w-auto me-3">
              ...
              <input type="text" class="form-control border-start-0" placeholder="Search unions..." style="width: 200px" :value="searchQuery" @input="(e) => handleSearch(e.target.value)" />
            </div>
            ...
            <button type="button" class="btn btn-sm btn-primary" @click="handleCreate"><i class="fa fa-plus me-1"></i> New Union</button>
          </div>
        </template>
          ...
          <template #card="{ row }">
            <div class="block block-rounded block-bordered h-100 mb-0 shadow-sm">
              <div class="block-content text-center py-4">
                ...
                <h4 class="mb-1">{{ row.union_name }}</h4>
                <p class="text-muted fs-sm mb-2">Rep: {{ row.contact_person }} | {{ row.phone }}</p>
                <div class="d-flex gap-2 justify-content-center mt-2">
                  <span class="badge bg-secondary">Dues: {{ row.monthly_dues || '0' }}</span>
                </div>
              </div>
