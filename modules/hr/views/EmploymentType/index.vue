...
const {
  ...
  initialSortBy: 'employment_type_id',
  initialSortDir: 'desc',
});
...
function rowId(row) {
  return row.id ?? row.employment_type_id ?? row[Object.keys(row).find((key) => key.endsWith('_id'))];
}
...
function handleView(row) {
  ...
  openFormModal('View EmploymentType', { ...row }, true);
}
...
function handleEdit(row) {
  ...
  openFormModal('Edit EmploymentType', { ...row }, false);
}
...
async function handleDelete(row) {
  ...
  handleResponseAlert(alertStore, responseData.value, 'EmploymentType deleted successfully.');
  await fetchRows();
}
...
async function handleSubmit(payload) {
  ...
  handleResponseAlert(
    ...
    responseData.value,
    isEdit
      ? 'EmploymentType updated successfully.'
      : 'EmploymentType created successfully.'
  );
...
<template>
  <div class="content">
    <DataTable
      ...
      search-placeholder="Search employment-type..."
      create-label="New EmploymentType"
      :paginated="true"
