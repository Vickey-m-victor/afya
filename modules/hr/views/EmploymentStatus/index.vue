...
const {
  ...
  initialSortBy: 'employment_status_id',
  initialSortDir: 'desc',
});
...
const tableColumns = [
  ...
  {
    "field": "is_active_state",
    "header": "Is Active State"
  }
];
...
function rowId(row) {
  return row.id ?? row.employment_status_id ?? row[Object.keys(row).find((key) => key.endsWith('_id'))];
}
...
function handleCreate() {
  ...
  if (!modalStore.useModal) {
    router.push({ name: 'hr/employment-status/create' });
    return;
  }

  openFormModal('Create EmploymentStatus', {}, false);
}
...
async function handleView(row) {
  ...
  if (!modalStore.useModal) return router.push({ name: 'hr/employment-status/view', params: { id } });
  if (!id) return alertStore.show({ theme: 'danger', type: 'toast', message: 'Record id not found.' });

  // 1. OPEN MODAL INSTANTLY
  openFormModal('View EmploymentStatus', {}, true);
...
async function handleEdit(row) {
  ...
  if (!modalStore.useModal) return router.push({ name: 'hr/employment-status/update', params: { id } });
  if (!id) return alertStore.show({ theme: 'danger', type: 'toast', message: 'Record id not found.' });

  // 1. OPEN MODAL INSTANTLY
  openFormModal('Edit EmploymentStatus', {}, false);
  modalStore.setLoading(true); // 💡 Global spinner ON
...
async function handleDelete(row) {
  ...
  handleResponseAlert(alertStore, responseData.value, isRestore ? 'EmploymentStatus restored successfully.' : 'EmploymentStatus deleted successfully.');
  await fetchRows();
}
...
async function handleSubmit(payload) {
  ...
  handleResponseAlert(
    ...
    responseData.value,
    isEdit
      ? 'EmploymentStatus updated successfully.'
      : 'EmploymentStatus created successfully.'
  );
...
<template>
  <div class="content">
    <DataTable
      ...
      search-placeholder="Search employment-status..."
      create-label="New EmploymentStatus"
      :paginated="true"
