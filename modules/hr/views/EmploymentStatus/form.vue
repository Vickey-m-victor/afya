...
<template>
    <form @submit.prevent="handleSubmit">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                <label class="form-label" for="field-employment_status_id">Employment status id</label>
                
                <input
                    id="field-employment_status_id"
                    v-model.number="formData.employment_status_id"
                    type="number"
                    
                    class="form-control"
                    :class="{ 'is-invalid': fieldError('employment_status_id') }"
                    :disabled="readonly || busy"
                />
                
                <div v-if="fieldError('employment_status_id')" class="invalid-feedback">{{ fieldError('employment_status_id') }}</div>
            </div>
            ...
            <div class="col-md-6">
                <label class="form-label" for="field-is_active_state">Is active state</label>
                
                <input
                    id="field-is_active_state"
                    v-model="formData.is_active_state"
                    type="text"
                    
                    class="form-control"
                    :class="{ 'is-invalid': fieldError('is_active_state') }"
                    :disabled="readonly || busy"
                />
                
                <div v-if="fieldError('is_active_state')" class="invalid-feedback">{{ fieldError('is_active_state') }}</div>
            </div>
        </div>
