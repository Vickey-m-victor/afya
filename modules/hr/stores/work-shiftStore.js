...
import workShiftService from "~/hr/services/workShiftService";
...
export const useWorkShiftStore = defineStore("workShift", {
  ...
  actions: {
    async fetchAll(searchQuery = "", page = 1, perPage = 25) {
      ...
      try {
        const response = await workShiftService.getAll(searchQuery, page, perPage);
    ...
    async create(payload) {
      const response = await workShiftService.create(payload);
      await this.fetchAll(); 
      return response;
    },
    
    async update(id, payload) {
      const response = await workShiftService.update(id, payload);
      await this.fetchAll(); 
      return response;
    },
