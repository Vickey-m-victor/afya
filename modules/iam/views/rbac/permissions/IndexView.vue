...
} = useDataTable({ initialSortBy: "permission_name", initialSortDir: "asc" });
...
const fetchPermissions = async () => {
  loading.value = true;
  try {
    const { data: responseData, request, error } = useApi("/iam/rbac/permissions", { method: "GET", autoFetch: false });
...
function handleSearch(query) { setSearchDebounced(query, fetchPermissions); }
function handlePageChange(page) { setPage(page); fetchPermissions(); }
function handlePerPageChange(value) { setPerPage(value); fetchPermissions(); }
function handleSort(field) { setSort(field); fetchPermissions(); }

function handleView(permission) {
  modalMode.value = "view";
  modalStore.toggleModalUsage(true); 
  if (!modalStore.useModal) return router.push({ name: 'iam/rbac/permissions/view', params: { id: permission.permission_id } });

  modalStore.openModal({
    component: PermissionsForm,
    props: { formData: { ...permission }, fieldErrors: {}, isLoading: false, readonly: true, hideSubmit: true, compact: true, onSubmit: handleSubmit },
    title: "View Permission", size: "md", showFooter: false, showConfirm: false, showCancel: false,
  });
}
...
function handleCreate() {
  ...
  modalStore.toggleModalUsage(true); 
  if (!modalStore.useModal) return router.push({ name: 'iam/rbac/permissions/create' });

  modalStore.openModal({
    component: PermissionsForm,
    props: { formData: {}, fieldErrors: {}, isLoading: false, readonly: false, hideSubmit: false, compact: true, onSubmit: handleSubmit },
    title: "Create Permission", size: "md", showFooter: false,
  });
}
...
function handleEdit(permission) {
  ...
  modalStore.toggleModalUsage(true); 
  if (!modalStore.useModal) return router.push({ name: 'iam/rbac/permissions/update', params: { id: permission.permission_id } });

  modalStore.openModal({
    component: PermissionsForm,
    props: { formData: { ...permission }, fieldErrors: {}, isLoading: false, readonly: false, hideSubmit: false, compact: true, onSubmit: handleSubmit },
    title: "Edit Permission", size: "md", showFooter: false,
  });
}

async function handleDelete(permission) {
  const result = await confirmAction(`Delete Permission "${permission.permission_name}"?`, "This will permanently remove the permission.");
  if (!result.isConfirmed) return;
  try {
    const { request, error } = useApi(`/iam/rbac/permission/${permission.permission_id}`, { method: "DELETE", autoFetch: false });
    await request();
    if (error.value) throw error.value;
...
function normalizeRoles(payloadData) {
  ...
  return rows.map((role) => ({ id: role.role_id || role.name, label: role.role_name || role.description || role.role_id || role.name }));
}
...
async function loadPermissionAssignments(permissionId) {
  const { data: rolesResponseData, request: fetchRolesReq, error: rolesError } = useApi("/iam/rbac/roles", { method: "GET", autoFetch: false });
  ...
  const roleAssignmentChecks = await Promise.all(
    roles.map(async (role) => {
      try {
        const { data: detailData, request: fetchRoleDetail, error: detailError } = useApi(`/iam/rbac/role/${role.id}`, { method: "GET", autoFetch: false });
        ...
        return { ...role, isAssigned: Object.prototype.hasOwnProperty.call(assignedMap, permissionId) };
      } catch {
  ...
  return { available: roleAssignmentChecks.filter((item) => !item.isAssigned), assigned: roleAssignmentChecks.filter((item) => item.isAssigned) };
}
...
async function handleManage(permission) {
  ...
  modalStore.openModal({
    ...
    size: "xl", showFooter: false, showConfirm: false, showCancel: false,
    props: {
      ...
      onAssign: async (selectedIds) => {
        ...
        try {
          const responses = await Promise.all(selectedIds.map(async (roleId) => {
            const { data: responseData, request, error } = useApi(`/iam/rbac/role/assign/${roleId}`, { method: "POST", autoFetch: false });
        ...
        } catch (err) { handleErrorAlert(err, "Failed to assign permission."); }
      ...
      onRemove: async (selectedIds) => {
        ...
        try {
          const responses = await Promise.all(selectedIds.map(async (roleId) => {
            const { data: responseData, request, error } = useApi(`/iam/rbac/role/remove/${roleId}`, { method: "POST", autoFetch: false });
            await request({ permissions: [permission.permission_id] });
            if (error.value) throw error.value;
            return responseData.value;
          }));
          responses.forEach((r) => handleResponseAlert(r, "Permission removed successfully."));
        } catch (err) { handleErrorAlert(err, "Failed to remove permission."); }
        await reloadPermissionAssignmentsIntoModal(permission.permission_id);
      },
    },
  });
...
async function handleSubmit(data) {
  ...
  if (modalMode.value === "create") {
    const { request, error } = useApi("/iam/rbac/permission", { method: "POST", autoFetch: false });
    await request(data);
    apiError = error.value;
  } else if (modalMode.value === "edit") {
    const { request, error } = useApi(`/iam/rbac/permission/${data.permission_id}`, { method: "PUT", autoFetch: false });
    await request(data);
    apiError = error.value;
  }
...
<template>
  <div class="content">
    ...
    >
      <template #cell-permission_name="{ row }">{{ row.permission_name }}</template>
      <template #cell-permission_id="{ row }"><code class="text-muted">{{ row.permission_id }}</code></template>
    </DataTable>
  </div>
</template>