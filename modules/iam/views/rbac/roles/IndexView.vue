...
} = useDataTable({ initialSortBy: "role_name", initialSortDir: "asc" });
...
const fetchRoles = async () => {
  loading.value = true;
  try {
    const { data: responseData, request, error } = useApi("/iam/rbac/roles", { method: "GET", autoFetch: false });
...
function handleSearch(query) { setSearchDebounced(query, fetchRoles); }
function handlePageChange(page) { setPage(page); fetchRoles(); }
function handlePerPageChange(value) { setPerPage(value); fetchRoles(); }
function handleSort(field) { setSort(field); fetchRoles(); }

function handleView(role) {
  modalMode.value = "view";
  modalStore.toggleModalUsage(true); 
  if (!modalStore.useModal) return router.push({ name: 'iam/rbac/roles/view', params: { id: role.role_id } });

  modalStore.openModal({
    component: RolesForm,
    props: { formData: { ...role }, fieldErrors: {}, isLoading: false, readonly: true, hideSubmit: true, compact: true, onSubmit: handleSubmit },
    title: "View Role", size: "md", showFooter: false, showConfirm: false, showCancel: false,
  });
}
...
function handleCreate() {
  ...
  modalStore.toggleModalUsage(true); 
  if (!modalStore.useModal) return router.push({ name: 'iam/rbac/roles/create' });

  modalStore.openModal({
    component: RolesForm,
    props: { formData: {}, fieldErrors: {}, isLoading: false, readonly: false, hideSubmit: false, compact: true, onSubmit: handleSubmit },
    title: "Create Role", size: "md", showFooter: false,
  });
}
...
function handleEdit(role) {
  ...
  modalStore.toggleModalUsage(true); 
  if (!modalStore.useModal) return router.push({ name: 'iam/rbac/roles/update', params: { id: role.role_id } });

  modalStore.openModal({
    component: RolesForm,
    props: { formData: { ...role }, fieldErrors: {}, isLoading: false, readonly: false, hideSubmit: false, compact: true, onSubmit: handleSubmit },
    title: "Edit Role", size: "md", showFooter: false,
  });
}

async function handleDelete(role) {
  const result = await confirmAction(`Delete Role "${role.role_name}"?`, "This will permanently remove the role and its permission assignments.");
  if (!result.isConfirmed) return;
  try {
    const { request, error } = useApi(`/iam/rbac/role/${role.role_id}`, { method: "DELETE", autoFetch: false });
    await request();
    if (error.value) throw error.value;
    await fetchRoles();
  } catch (error) {
...
async function loadRoleAssignments(roleId) {
  const { data: responseData, request, error } = useApi(`/iam/rbac/role/${roleId}`, { method: "GET", autoFetch: false });
  await request();
  if (error.value) throw error.value;
...
async function handleManagePermissions(role) {
  ...
  modalStore.openModal({
    ...
    props: {
      ...
      isSubmitting: false, availableLabel: "Available Permissions", assignedLabel: "Assigned Permissions",
      onAssign: async (selectedIds) => {
        modalStore.props.isSubmitting = true;
        const { data: responseData, request, error } = useApi(`/iam/rbac/role/assign/${role.role_id}`, { method: "POST", autoFetch: false });
        await request({ permissions: selectedIds });
        if (error.value) handleErrorAlert(error.value, "Failed to assign permissions.");
        else { handleResponseAlert(responseData.value, "Permissions assigned successfully."); await fetchRoles(); }
        await reloadRoleAssignmentsIntoModal(role.role_id);
      },
      onRemove: async (selectedIds) => {
        modalStore.props.isSubmitting = true;
        const { data: responseData, request, error } = useApi(`/iam/rbac/role/remove/${role.role_id}`, { method: "POST", autoFetch: false });
        await request({ permissions: selectedIds });
        if (error.value) handleErrorAlert(error.value, "Failed to remove permissions.");
        else { handleResponseAlert(responseData.value, "Permissions removed successfully."); await fetchRoles(); }
        await reloadRoleAssignmentsIntoModal(role.role_id);
      },
    },
  });
...
async function handleSubmit(data) {
  ...
  if (modalMode.value === "create") {
    const { request, error } = useApi("/iam/rbac/role", { method: "POST", autoFetch: false });
    await request(data);
    apiError = error.value;
  } else if (modalMode.value === "edit") {
    const { request, error } = useApi(`/iam/rbac/role/${data.role_id}`, { method: "PUT", autoFetch: false });
    await request(data);
    apiError = error.value;
  }
...
<template>
  <div class="content">
    ...
    >
      <template #cell-role_name="{ row }">{{ row.role_name }}</template>
      <template #cell-role_id="{ row }"><code class="text-muted">{{ row.role_id }}</code></template>
    </DataTable>
  </div>
</template>