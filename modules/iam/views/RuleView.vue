...
const formFields = reactive([
  { label: "Rule Name", type: "text", name: "rule_name", placeholder: "Enter Rule Name", col: "col-12" },
...
const handleSave = async () => {
  try {
    ...
    if (isEditing.value) { await store.update(recordId, formData.value); toastSuccess("Success", "Rule updated!"); }
    else { await store.create(formData.value); toastSuccess("Success", "Rule created!"); }
  ...
  <div class="content">
    ...
    >
      <template #header-actions>
        <button class="btn btn-sm btn-primary" @click="openCreateModal"><i class="fa fa-plus me-1"></i> Create Rule</button>
      </template>
    ...
    <BaseModal :showModal="showModal" size="modal-lg" :title="isEditing ? 'Edit Rule' : 'Create Rule'" @close="showModal = false">
