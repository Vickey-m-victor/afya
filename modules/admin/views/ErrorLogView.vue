...
const handleSave = async () => {
  try {
    if (isEditing.value) {
      ...
      const recordId = formData.value.id || formData.value.uuid || formData.value[`error_log_id`];
      await store.update(recordId, formData.value);
      toastSuccess("Success", "ErrorLog updated successfully!");
    } else {
      await store.create(formData.value);
      toastSuccess("Success", "ErrorLog created successfully!");
    }
...
<template>
  <div class="content">
    ...
    <BaseModal 
      :showModal="showModal" 
      :title="isEditing ? 'Edit ErrorLog' : 'Create ErrorLog'" 
