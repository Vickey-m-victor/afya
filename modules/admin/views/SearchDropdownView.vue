...
import { useSearchDropdownStore } from "~/admin/stores/searchDropdownStore";
import { useAlert } from "@/composables/alerts";

const { toastSuccess, toastError, confirmAction } = useAlert();
const store = useSearchDropdownStore();
...
const handleDelete = async (row) => {
  ...
  const recordId = row.id || row.uuid || row[`search_dropdown_id`]; 
...
const handleSave = async () => {
  try {
    // Adjust if your backend uses a custom ID name
    const recordId = formData.value.id || formData.value.uuid || formData.value[`search_dropdown_id`];
    if (isEditing.value) {
      await store.update(recordId, formData.value);
      toastSuccess("Success", "SearchDropdown updated successfully!");
    } else {
      await store.create(formData.value);
      toastSuccess("Success", "SearchDropdown created successfully!");
    }
...
<template>
  <div class="content">
    <BasePageHeading title="SearchDropdown Management" />

    <BaseTable
      title="SearchDropdowns"
      :data="store.items"
    ...
    >
      <template #header-actions>
        <BaseButton label="Create SearchDropdown" variant="primary" @click="openCreateModal" />
      </template>
    ...
    <BaseModal 
      :showModal="showModal" 
      :title="isEditing ? 'Edit SearchDropdown' : 'Create SearchDropdown'" 
