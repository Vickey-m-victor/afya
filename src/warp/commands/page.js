...
import { generatePage } from '../lib/generators/pageGenerator.js';
...
pageCommand
    ...
    .action(() => {
        console.log(`
  ${chalk.green.bold('warp page')}
  ${chalk.gray('--------------------------------')}
  Usage:
    ${chalk.green('npm run warp -- page create --name=PageName --path=./src/pages')}
...
pageCommand
    ...
    .action((options) => {
        ...
        generatePage(options.name, options.path);
    });
